...
public class Application extends FBController {
	...
	public static void index(){
		...
		String startDate = XPath.selectText("started", showNode);	
		...
		for(Node node: XPath.selectNodes("Results/show[1]/genres", document)) {
			genres += XPath.selectText("genre", node) +"-";
		}

		Series series = new Series(id,name,startDate,totalSeasons,genres,1);
		...
		for (int i = 1; i <= series.totalSeasons; i++) {
			for(Node showNode2 : XPath.selectNodes("Show/Episodelist/Season[@no='"+i+"']/episode", document2)){
				String epnum = XPath.selectText("epnum", showNode2);
				String seasonnum = XPath.selectText("seasonnum", showNode2);
				String airdate = XPath.selectText("airdate", showNode2);
				String title = XPath.selectText("title", showNode2);
				String rating = XPath.selectText("rating", showNode2);

				Season season = new Season(i, series.getId());
				Episode ep = new Episode(title, Integer.valueOf(seasonnum), airdate, rating, season.getId());
			}
		}
